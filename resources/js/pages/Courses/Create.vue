        ...
        <form @submit.prevent="submit" class="space-y-6">
          ...
          <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3">
            <Link :href="route('courses.index')" 
                  class="w-full sm:w-auto text-center bg-gray-300 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-400 transition-colors">
              取消
            </Link>
            <button
              type="submit"
              :disabled="form.processing"
              class="w-full sm:w-auto bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors disabled:opacity-50"
            >
              <span v-if="form.processing">發布中...</span>
